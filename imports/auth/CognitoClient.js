...
class CognitoClient {
  ...
  receiveTokens(tokens) {
    ...
    this.fetchUserInfo(tokens.access_token)
      ...
      .then(value => { this.userInfo = value;  Session.set(sessionKeys.userInfo, value); }, logError);
  }
  ...
  removeCodeFromUrl = () => window.history.replaceState({}, window.title, window.location.origin);
  ...
  tryLogin() {
    ...
    if (code) {
      ...
      window.location = cognitoClient.loginUrl();
    }
  }

  loggedIn = () => Session.get(sessionKeys.userInfo) != null;
